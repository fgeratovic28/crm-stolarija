-- Workflow update:
-- 1) Otkazan RN merenja/ugradnje -> automatski novi RN istog tipa (pending) sa razlogom otkazivanja.
-- 2) Problem na ugradnji (everything_ok=false ili nedostaju stavke) -> novi RN ugradnje + status "installation_problem".
-- 3) Otkaz + problem na ugradnji -> isto kao #2, uz mogućnost ponovnog zakazivanja datuma na novom RN.
...
BEGIN
  ...
  v_has_problem_content := (NOT v_everything_ok) OR v_has_missing OR v_has_additional OR v_issues IS NOT NULL;

  -- ----------------------------
  -- A) OTKAZAN TEREN (merenje / ugradnja): kreiraj novi RN istog tipa
  -- ----------------------------
  IF v_site_canceled
     AND v_wo_type IN (
    ...
    -- Status posla:
    -- - ugradnja + problem => installation_problem
    -- - ugradnja bez problema => korak nazad (scheduled)
    -- - merenje => korak nazad (measuring)
  ...
  -- ----------------------------
  -- B) Problem na ugradnji (i bez otkaza): novi RN ugradnje + installation_problem
  -- ----------------------------
  ...
  -- ----------------------------
  -- C) Ostalo: zadrži postojeće ponašanje
  -- ----------------------------
