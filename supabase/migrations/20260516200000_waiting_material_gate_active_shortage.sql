-- Posao mora ostati na „Čeka materijal" sve dok je bilo koja Porudžbina po nedostatku
-- (`material_orders.is_shortage_order = true`) aktivna — tj. dok nije u statusu
-- `materials_received`. Tek pošto je shortage stvarno primljen (ili refundiran pa zatvoren),
-- job ide u sledeću fazu (`in_production`, `scheduled`...).

CREATE OR REPLACE FUNCTION public.job_has_active_shortage_orders(p_job_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.job_has_active_shortage_orders(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.job_has_active_shortage_orders(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.job_has_active_shortage_orders(uuid) TO service_role;

COMMENT ON FUNCTION public.job_has_active_shortage_orders(uuid) IS
  'TRUE ako posao ima Porudžbinu po nedostatku koja nije u statusu materials_received (još nije primljena ili zatvorena).';
...
BEGIN
  ...
  ELSIF v_measurement_completed AND v_has_material_order AND v_all_materials_delivered AND v_has_active_shortage_orders THEN
    -- Porudžbina po nedostatku još nije „primljena" (materials_received) — drži posao u „Čeka materijal".
    v_next := 'waiting_material'::public.job_status;
...
-- Bonus: kada se shortage zatvori (delivery_status pređe u materials_received), pokreni recompute
-- za odgovarajući job. Trigger se okida na bilo koju izmenu material_orders.delivery_status.
CREATE OR REPLACE FUNCTION public.trg_material_orders_recompute_job_on_delivery_change()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF TG_OP = 'UPDATE'
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE WARNING 'recompute_job_status nakon promene delivery_status: %', SQLERRM;
    END;
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS material_orders_recompute_job_on_delivery_change ON public.material_orders;
CREATE TRIGGER material_orders_recompute_job_on_delivery_change
  AFTER UPDATE OF delivery_status ON public.material_orders
  FOR EACH ROW
  EXECUTE FUNCTION public.trg_material_orders_recompute_job_on_delivery_change();

REVOKE ALL ON FUNCTION public.trg_material_orders_recompute_job_on_delivery_change() FROM PUBLIC;
