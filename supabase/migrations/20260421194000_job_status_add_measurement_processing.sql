-- DOPUNA FAZE 1: dodavanje statusa "Obrada mera" (measurement_processing).
-- Minimalna izmena: proširenje enum-a i ažuriranje postojećih funkcija
-- koje mapiraju/zaključavaju ručne statuse i SLA poruke.

ALTER TYPE public.job_status ADD VALUE IF NOT EXISTS 'measurement_processing';
...
BEGIN
  ...
  IF v_locked OR v_current IN (
    ...
    'ready_for_work'::public.job_status,
    'waiting_material'::public.job_status,
    'canceled'::public.job_status
  ) THEN
  ...
  SELECT
    ...
    '[AUTO] SLA upozorenje: Posao ' || c.job_number
      ...
      || CASE c.status::text
        ...
        WHEN 'accepted' THEN 'Prihvaćeno'
        WHEN 'measuring' THEN 'Merenje'
        WHEN 'measurement_processing' THEN 'Obrada mera'
        WHEN 'ready_for_work' THEN 'Spremno za rad'
        WHEN 'waiting_material' THEN 'Čeka materijal'
        WHEN 'in_production' THEN 'U proizvodnji'
        WHEN 'installation_in_progress' THEN 'Ugradnja u toku'
        WHEN 'scheduled' THEN 'Čeka ugradnju'
        WHEN 'canceled' THEN 'Otkazan'
        ELSE c.status::text
      END
