-- Finalna ponuda: checkbox is_final pri kreiranju vezuje post-merni tok i status posla.

CREATE OR REPLACE FUNCTION public.quote_in_post_measurement_cycle(
  p_is_final boolean,
  p_created_at timestamptz,
  p_version_number integer,
  p_meas_finished_at timestamptz
)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT COALESCE(p_is_final, false)
    OR (
      p_meas_finished_at IS NOT NULL
      AND p_created_at >= p_meas_finished_at
    )
    OR (
      p_meas_finished_at IS NULL
      AND COALESCE(p_version_number, 1) > 1
    );
$$;
...
BEGIN
  ...
  ELSIF NEW.status = 'sent'::public.quote_status
        AND (
          v_job_status = 'measurement_processing'::public.job_status
          OR COALESCE(NEW.is_final, false)
        ) THEN
    UPDATE public.jobs
    ...
    WHERE id = NEW.job_id
      AND status IN (
        ...
        'measurement_processing'::public.job_status,
        'accepted'::public.job_status
      );
  ELSIF NEW.status = 'accepted'::public.quote_status THEN
    IF COALESCE(NEW.is_final, false) THEN
      UPDATE public.jobs
      SET status = 'final_quote_accepted_pending_payment'::public.job_status
      WHERE id = NEW.job_id
        AND status IN (
...
BEGIN
  ...
  v_post_meas_quote_accepted := EXISTS (
    ...
    WHERE q.job_id = p_job_id
      AND q.status = 'accepted'::public.quote_status
      AND (
        COALESCE(q.is_final, false)
        OR (v_meas_finished_at IS NOT NULL AND q.created_at >= v_meas_finished_at)
