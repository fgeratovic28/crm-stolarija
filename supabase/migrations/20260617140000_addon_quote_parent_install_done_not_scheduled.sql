-- Upit za novu ponudu sa ugradnje (izveštaj u redu): parent ostaje u redovnom toku
-- (ugradnja završena / nije plaćeno ili završen). Prodaja dobija alert; status posla ne ide na Zakazano.
...
COMMENT ON FUNCTION public.apply_field_report_workflow_branching(uuid) IS
  'Grananje terenskog izveštaja. Upit za novu ponudu (addon_quote + sve u redu): bez RN/status problema; recompute vodi u ugradnja završena/završen.';
...
-- Ispravi status parent posla pogrešno postavljen zbog upita za novu ponudu (redovan završetak ugradnje).
DO $backfill$
DECLARE
  r record;
BEGIN
  FOR r IN
    ...
    FROM public.jobs j
    WHERE j.status IN (
      'installation_problem'::public.job_status,
      'scheduled'::public.job_status
    )
    AND (
      EXISTS (
  ...
  LOOP
    PERFORM public.recompute_job_status(r.job_id);
  END LOOP;
END;
$backfill$;
