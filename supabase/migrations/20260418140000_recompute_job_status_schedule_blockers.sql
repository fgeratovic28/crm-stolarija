-- Pending RN koji blokiraju „Čeka ugradnju“: sve osim ugradnje i opcionih terenskih poseta/kontrola.
...
DECLARE
  ...
  v_has_schedule_blocker_pending boolean;
  v_has_meas boolean;
...
BEGIN
  ...
  v_has_schedule_blocker_pending := EXISTS (
    SELECT 1 FROM public.work_orders w
    WHERE w.job_id = p_job_id
      AND w.status = 'pending'::work_order_status
      AND w.type NOT IN (
        'installation'::work_order_type,
        'site_visit'::work_order_type,
        'control_visit'::work_order_type
      )
  );
  ...
  ELSIF v_has_any_pending THEN
    IF v_meas_phase_done AND NOT v_inst_job_done AND NOT v_has_schedule_blocker_pending THEN
