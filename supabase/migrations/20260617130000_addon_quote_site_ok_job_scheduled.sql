-- Upit za novu ponudu sa ugradnje (izveštaj u redu): posao ide na „Zakazano“, ne na „Ugradnja – problem“ / Završen.
...
BEGIN
  ...
  IF v_addon_quote AND v_wo_type = 'installation'::public.work_order_type AND v_has_additional THEN
    ...
    IF v_everything_ok THEN
      UPDATE public.jobs
      SET
        status = 'scheduled'::public.job_status,
        status_changed_at = NOW()
      WHERE id = v_job_id
        AND status IS DISTINCT FROM 'scheduled'::public.job_status;
    END IF;
  END IF;
END;
$apply$;
...
COMMENT ON FUNCTION public.apply_field_report_workflow_branching(uuid) IS
  'Grananje terenskog izveštaja. Upit za novu ponudu (addon_quote + sve u redu): Zakazano, bez RN problema.';

-- Posle zakazivanja dopune ugradnje (predračun / problem): ne vraćaj posao u installation_problem
-- dok postoji otvoren RN ugradnje (pending / u toku).
...
DECLARE
  ...
  v_has_open_addon_quote_alert boolean;
  v_install_ok_addon_quote_report boolean;
BEGIN
  ...
  v_has_open_addon_quote_alert := EXISTS (
    SELECT 1
  ...
  v_install_ok_addon_quote_report := EXISTS (
    SELECT 1
    FROM public.field_reports fr
    INNER JOIN public.work_orders w ON w.id = fr.work_order_id
    WHERE w.job_id = p_job_id
      AND w.type = 'installation'::public.work_order_type
      AND w.status = 'completed'::public.work_order_status
      AND COALESCE(fr.addon_quote_site_request, false) = true
      AND COALESCE(fr.everything_ok, true) = true
      AND COALESCE(fr.site_canceled, false) = false
  );
  ...
  ELSIF v_has_inst
    ...
    v_next := 'installation_problem'::public.job_status;
  ELSIF (v_has_open_addon_quote_alert OR v_install_ok_addon_quote_report)
    AND NOT v_inst_in_progress THEN
...
-- Poslovi pogrešno u „Ugradnja – problem“ zbog upita za novu ponudu (izveštaj u redu).
UPDATE public.jobs j
SET
  status = 'scheduled'::public.job_status,
  status_changed_at = NOW()
WHERE j.status = 'installation_problem'::public.job_status
  AND (
    EXISTS (
      SELECT 1
      FROM public.sales_site_addon_quote_alerts a
