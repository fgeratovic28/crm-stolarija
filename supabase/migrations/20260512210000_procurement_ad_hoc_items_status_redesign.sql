-- Vanredne stavke (procurement_ad_hoc_items): novi set statusa, nezavisan od reklamacija.
-- Životni ciklus: needs_order  →  ordered  →  received
-- Terminal: canceled
-- Aktivni (broje se kao „čeka prijem"): needs_order, ordered.

-- 1) Prvo skidamo stari CHECK constraint (i default) jer UPDATE niže prevodi stare vrednosti
--    u nove koje stari constraint ne dozvoljava (`received`, `needs_order`...).
...
-- 2) Prevod postojećih vrednosti u nove (sada radi jer constraint više ne važi).
...
-- 3) Novi CHECK constraint i nova podrazumevana vrednost.
ALTER TABLE public.procurement_ad_hoc_items
...
COMMENT ON COLUMN public.procurement_ad_hoc_items.status IS
  'needs_order | ordered | received | canceled';

-- 3) create_procurement_ad_hoc_item: nova početna vrednost statusa.
...
-- 4) receive_procurement_barcode: za ad-hoc deo koristiti nove statuse.
...
BEGIN
  ...
  -- Reklamacije (C-barkod) — nepromenjen tok (njihovi statusi ostaju kao kod reklamacija).
  ...
  IF v_id IS NOT NULL THEN
    ...
    -- Auto-flip delivery_status na materials_received ako više nema otvorenih reklamacija.
  ...
  -- Vanredne stavke (A-barkod) — koristi novi status set.
...
-- 5) get_procurement_order_public_redirect: koristi nove aktivne statuse za ad-hoc deo.
