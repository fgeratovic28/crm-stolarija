-- Baner „Prihvaćeno / merenje“: posao ostaje na listi dok god postoji bar jedan
-- pending/in_progress RN merenja bez PUNOG zakazivanja (tim + datum), ne samo dok
-- ne postoji nijedan „kompletan“ RN (više aktivnih RN-ova / ponovno merenje).
-- LEFT JOIN na kupca da posao ne nestane ako je veza oštećena.
...
BEGIN
  ...
  LEFT JOIN LATERAL (
    ...
    ORDER BY
      (wo.team_id IS NULL) DESC,
      (wo.date IS NULL) DESC,
      wo.date NULLS LAST,
      wo.id DESC
    LIMIT 1
  ) w ON true
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE wx.job_id = j.id
          ...
          AND (wx.team_id IS NULL OR wx.date IS NULL)
      )
    )
...
COMMENT ON FUNCTION public.list_accepted_needs_measurement_schedule_for_dashboard() IS
  'Dashboard: Prihvaćeno/Merenje — posao na listi dok postoji pending RN merenja bez tima ili bez datuma (bilo koji).';
