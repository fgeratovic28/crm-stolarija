-- RN dopune ugradnje tek kada nema više nepročitanih hitnih obaveštenja za druge pozicije istog posla
-- (isti skup „nedostatak sa predračuna na terenu“). Do tada: red u invoice_missing_part_secured sa followup_work_order_id NULL.
-- Za bulk magacin: p_defer_peer_position_keys = sve pozicije iz istog poziva da se ne blokiraju međusobno.

DROP FUNCTION IF EXISTS public.invoice_missing_part_secure_core(uuid, text, uuid);
DROP FUNCTION IF EXISTS public.invoice_missing_part_secure_core(uuid, text, uuid, boolean);
...
ALTER FUNCTION public.invoice_missing_part_secure_core(uuid, text, uuid, boolean, text[]) SET search_path TO public;
REVOKE ALL ON FUNCTION public.invoice_missing_part_secure_core(uuid, text, uuid, boolean, text[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.invoice_missing_part_secure_core(uuid, text, uuid, boolean, text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.invoice_missing_part_secure_core(uuid, text, uuid, boolean, text[]) TO service_role;

COMMENT ON FUNCTION public.invoice_missing_part_secure_core(uuid, text, uuid, boolean, text[]) IS
  'Magacin / prijem / proizvodnja: RN dopune. Ako p_defer_until_all_hitni_positions_ready i još ima nepročitanih hitnih alertova za druge pozicije (van p_defer_peer_position_keys), samo secured sa followup NULL — bez read; jedan RN i read svih hitnih za posao kada poslednji skup bude zatvoren.';
...
BEGIN
  ...
  SELECT coalesce(array_agg(DISTINCT lower(trim(both ' ' FROM t))), array[]::text[])
  INTO v_peer_keys
  FROM unnest(v_list) AS u(t);
  ...
  LOOP
    v_pos := v_list[v_i];
    v_ret := public.invoice_missing_part_secure_core(
      p_job_id,
      v_pos,
      v_shared,
      true,
    ...
    IF v_shared IS NULL AND v_ret IS NOT NULL THEN
      v_shared := v_ret;
    END IF;
    v_i := v_i + 1;
  END LOOP;
...
DROP FUNCTION IF EXISTS public.invoice_missing_finalize_deferred_followup_wo(uuid);
