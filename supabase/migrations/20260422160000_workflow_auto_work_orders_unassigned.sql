-- Automatski RN (merenje / proizvodnja / ugradnja) bez tima dok dispečer ne dodeli tim.
-- Uklanja CHECK koji je bio NOT VALID, dodaje RLS za office, i RPC koji kreira naloge idempotentno.

ALTER TABLE public.work_orders
  DROP CONSTRAINT IF EXISTS work_orders_team_id_required;
...
BEGIN
  ...
  -- Otvoren = pending ili in_progress
  SELECT EXISTS (
    SELECT 1
  ...
  -- 1) Merenje: posao u fazi „Merenje“ (bez duplog ako merenje već ima završen RN)
  ...
  -- 2) Proizvodnja: završen RN merenja + terenski izveštaj „sve u redu“ (bez otkazivanja na terenu)
  SELECT EXISTS (
    SELECT 1
  ...
  IF v_meas_ok_for_prod
    ...
    SELECT LEFT(
      trim(both E'\n' FROM concat_ws(
        ...
        NULLIF(trim(both ' ' FROM fr.additional_needs), ''),
  ...
  -- 3) Ugradnja: završen RN proizvodnje; procena sati iz merenja
  SELECT EXISTS (
    SELECT 1
    FROM public.work_orders w
    WHERE w.job_id = p_job_id
